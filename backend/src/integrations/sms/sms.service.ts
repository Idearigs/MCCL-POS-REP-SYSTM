...
import { SmsProcessorService, SMSProcessorResult } from './sms-processor.service';
...
export class SmsService {
  ...
  async sendSMS(params: {
    ...
    try {
      ...
      const requestData = {
        to: cleanPhone,
        from: params.from || this.configService.get('VOODOOSMS_SENDER_ID', 'MPS Jewel'),
        msg: params.message,
  ...
  private generateRepairStatusMessage(data: RepairStatusSMSData): string {
    ...
    let message = statusMessages[data.newStatus as keyof typeof statusMessages] || 
    ...
    if (data.estimatedCompletionDate && ['IN_PROGRESS', 'APPROVED', 'WAITING_PARTS'].includes(data.newStatus)) {
