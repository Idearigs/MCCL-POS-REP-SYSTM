...
@Injectable()
export class CredentialsExportService {
  ...
  /**
   * Generate HTML document for credentials
   */
  async generateHtmlDocument(profileId: string): Promise<string> {
  ...
  async createUserWithCredentials(
    ...
    const user = await this.prisma.mf_customer_users.create({
      data: {
        ...
        role: role as any,
        passwordHash,
        tempPassword, // Store temporarily for credential export
