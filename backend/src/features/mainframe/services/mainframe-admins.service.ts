...
export class MainframeAdminsService {
  ...
  async create(data: { firstName: string; lastName: string; email: string; password: string; role?: string }) {
    const existing = await this.prisma.mf_admins.findUnique({ where: { email: data.email.toLowerCase() } });
  ...
  async login(email: string, password: string) {
    const admin = await this.prisma.mf_admins.findUnique({ where: { email: email.toLowerCase() } });
    if (!admin || !admin.isActive) throw new UnauthorizedException('Invalid credentials');

    const passwordHash = await this.hashPassword(password);
    if (passwordHash !== admin.passwordHash) throw new UnauthorizedException('Invalid credentials');

    await this.prisma.mf_admins.update({ where: { id: admin.id }, data: { lastLoginAt: new Date() } });
    return { id: admin.id, firstName: admin.firstName, lastName: admin.lastName, email: admin.email, role: admin.role };
  }

  async update(id: string, data: { firstName?: string; lastName?: string; role?: string; isActive?: boolean }) {
